...
leavesRouter.post('/', async (req: AuthedRequest, res, next) => {
  try {
    ...
    // Notify admins about new leave request (in-app toast + sound + notification bell + push)
    if (req.user?.role === 'team') {
      const admins = await UserModel.find({ role: { $in: ['admin', 'super-admin'] }, status: 'active' }).select('_id').lean();
      const message = `${created.userName || 'Employee'} requested ${created.type || 'leave'} (${created.startDate || ''}${created.endDate && created.endDate !== created.startDate ? ` to ${created.endDate}` : ''}). Reason: ${(created.reason || '').slice(0, 80)}${(created.reason || '').length > 80 ? '…' : ''}`;
      for (const admin of admins) {
        const adminId = String(admin._id);
        emitNotify(adminId, {
          id: `leave-request-${created.id}`,
        ...
        sendPushToUser(adminId, {
          title: 'New Leave Request',
          body: `${created.userName || 'Employee'} requested ${created.type || 'leave'} (${created.startDate || ''})`,
